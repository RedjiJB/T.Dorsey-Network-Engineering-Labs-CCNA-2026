# Day 23: OSPF Basics & Neighbor Adjacency (Black Start)
## 0. Metadata
- **Objective:** Deploy offline-resilient OSPF Basics & Neighbor Adjacency with minimal external dependencies
- **Research Field:** Field-1: Black Start
- **Proof:** OSPF Basics & Neighbor Adjacency functions offline; no dynamic updates until power restored
- **Haiti Phase:** P38 (offline resilience)
- **Difficulty:** Advanced
- **Time:** 120 minutes

## 1. Business Context
Haiti P38 experiences frequent power outages (avg. 6 hours/day). OSPF Basics & Neighbor Adjacency must operate in offline mode with pre-cached configurations. This lab validates that OSPF Basics & Neighbor Adjacency remains stable without external updates, and quickly re-converges when power restores.
...
OSPF Basics & Neighbor Adjacency must operate without external dependencies during extended offline periods. Cached configurations are critical for rural deployment resilience.
...
50 P38 sites, each with cached OSPF Basics & Neighbor Adjacency config; no centralized controller.
...
- **BSL-1:** Cache OSPF Basics & Neighbor Adjacency config, simulate offline, verify operation
...
**Conclusion:** OSPF Basics & Neighbor Adjacency offline resilience proven for P38. Cached configurations enable autonomous operation during extended power loss.
