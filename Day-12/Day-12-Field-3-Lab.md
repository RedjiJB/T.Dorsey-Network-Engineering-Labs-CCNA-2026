# Day 12: Basic Routing (Inter-VLAN) (Field 3: DePIN)

## 0. Metadata

- **Objective:** Prove Basic Routing (Inter-VLAN) functionality in DePIN (Field 3) constraints
...
- **Prerequisites:** Day 12 base lab manual + Field 3 environment setup
...
- **Key Concepts:** Field-optimized Basic Routing (Inter-VLAN), DePIN-specific validation, proof obligations, real-world constraints

## 1. Business Context

DePIN (Decentralized Physical Infrastructure Networks) rely on **mesh topologies** without central authority.
This lab proves Basic Routing (Inter-VLAN) works in **full-mesh, Byzantine-fault-tolerant** design:
- No central hub or single point of failure
- Nodes must reach consensus on Basic Routing (Inter-VLAN) without a leader
- Byzantine nodes (malicious or failed) must not break the network
- Voting quorum (n/2 + 1) must validate all state changes

**Why this matters:** Decentralized networks cannot rely on centralized control. Basic Routing (Inter-VLAN) must be fault-tolerant at the core.
...
- Test Basic Routing (Inter-VLAN) with Byzantine nodes (R4 offline or lying)
...
- Implement voting/consensus for Basic Routing (Inter-VLAN) decisions
...
|--------|------|--------|---------|------|-------|
| R1 | Primary | 10.0.12.0/24 | 10.0.12.1 | /24 | Field 3 core router |
...
Router(config-if)# ip address 10.0.12.11 255.255.255.0
Router(config-if)# no shutdown
Router(config-if)# exit

Router(config)# int g0/2
Router(config-if)# ip address 10.0.12.21 255.255.255.0
Router(config-if)# no shutdown
Router(config-if)# exit

! Implement voting/consensus (pseudo-code):
! if (consensus_check(Basic Routing (Inter-VLAN)) >= n/2 + 1):
!     accept_update()
! else:
!     reject_update()
...
C 10.0.12.21/24 is directly connected, GigabitEthernet0/2
...
PC1# ping 10.0.12.10
Reply from 10.0.12.10: bytes=32 time=7ms
...
**Why mesh topology for Basic Routing (Inter-VLAN)?**
...
**This lab validates:** Basic Routing (Inter-VLAN) works in Byzantine-fault-tolerant mesh
