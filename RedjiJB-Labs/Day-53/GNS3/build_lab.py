#!/usr/bin/env python3
"""
GNS3 automation script for Day 53 - GRE Tunnels.

Builds the R1 / R2 / SPR1 / SPR2 / SW1 / SW2 / PC1 / PC2 topology from
Day-53-Lab-Manual.md using free, open-source images:

    Role                      Image
    ------------------------  ----------------------------
    Routers (R1, R2, SPR1,    VyOS
    SPR2)
    Switches (SW1, SW2)       Open vSwitch (GNS3 built-in)
    PCs (PC1, PC2)            Alpine Linux

VyOS supports GRE tunnel interfaces natively and OSPF via FRRouting, so
this lab translates to the open-source stack with full functional parity
-- unlike some later security-focused labs, no feature gap here.
...
PROJECT_NAME = "Day-53-GRE-Tunnels"

REQUIRED_TEMPLATES = {
    "VyOS": {
        "download_url": "https://s3.amazonaws.com/s3-us.vyos.io/rolling/current/vyos-rolling-latest.iso",
        "note": "Router role (R1, R2, SPR1, SPR2). Open-source, Cisco-like CLI, "
        "supports GRE tunnel interfaces and OSPF (FRRouting) natively.",
    },
    "Open vSwitch": {
        "download_url": None,
...
NODES = [
    # Office A
    {"name": "PC1", "template": "Alpine Linux", "x": -500, "y": -100},
    {"name": "SW1", "template": "Open vSwitch", "x": -350, "y": -100},
    {"name": "R1", "template": "VyOS", "x": -200, "y": -100},
    # Service provider underlay
    {"name": "SPR1", "template": "VyOS", "x": -50, "y": -100},
    {"name": "SPR2", "template": "VyOS", "x": 150, "y": -100},
    # Office B
    {"name": "R2", "template": "VyOS", "x": 300, "y": -100},
    {"name": "SW2", "template": "Open vSwitch", "x": 450, "y": -100},
    {"name": "PC2", "template": "Alpine Linux", "x": 600, "y": -100},
]

# Physical/underlay links only. The GRE tunnel itself is a logical overlay
# configured inside VyOS (interfaces tunnel tun0) once the underlay is up --
# it is not a GNS3 link and has nothing to create here.
LINKS = [
    ("PC1", "SW1"),
    ("SW1", "R1"),
    ("R1", "SPR1"),
...
def main():
    ...
    print(
        f"\nDay 53 topology built in project '{PROJECT_NAME}'. "
        "Open it in the GNS3 GUI, bring up the underlay addressing first, "
        "then configure the GRE tunnel (VyOS 'interfaces tunnel tun0') "
        "and OSPF on R1 and R2 -- see GNS3/README.md."
    )
