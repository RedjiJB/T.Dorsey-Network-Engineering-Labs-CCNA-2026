#!/usr/bin/env python3
"""
GNS3 automation script for Day 33 - IPv6 Static Routes, SLAAC, and Backup Paths.

Builds a three-router topology: R1 and R3 are directly linked (primary path)
and both also connect via serial links to R2 (backup path), using free,
open-source images:
...
PROJECT_NAME = "Day-33-IPv6-Static-Routes-Backup"

REQUIRED_TEMPLATES = {
    "VyOS": {
        "download_url": "https://s3.amazonaws.com/s3-us.vyos.io/rolling/current/vyos-rolling-latest.iso",
        "note": "Router role (R1, R2, R3). Supports IPv6 static routing and RA/SLAAC.",
    },
    "Alpine Linux": {
        "download_url": "https://dl-cdn.alpinelinux.org/alpine/latest-stable/releases/x86_64/",
        "note": "End devices (PC1, PC2). Supports SLAAC natively.",
    },
    "Open vSwitch": {
        "download_url": None,
        "note": "Switch role (SW1, SW2). Built into GNS3 - no download needed.",
    },
}

NODES = [
    {"name": "PC1", "template": "Alpine Linux", "x": -600, "y": -100},
    {"name": "SW1", "template": "Open vSwitch", "x": -450, "y": -100},
    {"name": "R1", "template": "VyOS", "x": -250, "y": -100},
    {"name": "R2", "template": "VyOS", "x": 0, "y": 100},
    {"name": "R3", "template": "VyOS", "x": 250, "y": -100},
...
LINKS = [
    ("PC1", "SW1"),
    ("SW1", "R1"),
    ("R1", "R3"),   # primary direct link
    ("R1", "R2"),   # backup path leg 1 (serial, link-local only)
    ("R2", "R3"),   # backup path leg 2 (serial, link-local only)
    ("R3", "SW2"),
    ("SW2", "PC2"),
]
...
def main():
    ...
    print(
        f"\nDay 33 topology built in project '{PROJECT_NAME}'. "
