#!/usr/bin/env python3
"""
GNS3 automation script for Day 03 - OSI Model & DHCP Packet Analysis.

Builds the client/DHCP-server topology from
Labs/Day-03-OSI-Model-DHCP-Packet-Analysis.md using free, open-source images:

    Role                Image
    ------------------  ----------------------------
    Routers (R1, R2)    VyOS
    Switches (SW1, SW2) Open vSwitch (GNS3 built-in)
    PC1 / SRV1          Linux (Alpine)
...
NODES = [
    {"name": "PC1", "template": "Alpine Linux", "x": -400, "y": 0},
...
def main():
    ...
    print(
        f"\nDay 03 topology built in project '{PROJECT_NAME}'. "
        "Open it in the GNS3 GUI to start the nodes. Configure R1 as a DHCP "
        "relay (VyOS: 'set service dhcp-relay interface eth0' + "
        "'set service dhcp-relay server <SRV1-IP>'), enable udhcpd on SRV1, "
        "and capture the exchange with Wireshark on the R1-R2 link to "
        "replicate the DORA packet inspection from the companion manual."
    )
