#!/usr/bin/env python3
"""
GNS3 automation script for Day 18 - Multilayer Switching: SVIs and
Inter-VLAN Routing.

Builds the SW1/SW2/R1 multilayer-switching topology using free,
open-source images:

    Role                        Image
    --------------------------  ----------------------------
    Router (R1)                 VyOS
    Access switch (SW1)         Open vSwitch (GNS3 built-in)
    Multilayer switch (SW2)     Open vSwitch (GNS3 built-in) -- see README
                                 for its Layer 3 SVI limitations
    PCs (PC1-PC7)                Linux (Alpine)
...
PROJECT_NAME = "Day-18-Multilayer-Switching"

REQUIRED_TEMPLATES = {
    "VyOS": {
        "download_url": "https://s3.amazonaws.com/s3-us.vyos.io/rolling/current/vyos-rolling-latest.iso",
        "note": "Internet edge router role (R1).",
    },
    "Alpine Linux": {
        "download_url": "https://dl-cdn.alpinelinux.org/alpine/latest-stable/releases/x86_64/",
        "note": "End devices (PC1-PC7).",
    },
    "Open vSwitch": {
        "download_url": None,
        "note": "Access switch (SW1) and multilayer-switch stand-in (SW2). "
        "Built into GNS3 -- no image download needed. See README for SVI caveats.",
    },
}

NODES = [
    {"name": "PC1", "template": "Alpine Linux", "x": -600, "y": -200},
    {"name": "PC2", "template": "Alpine Linux", "x": -600, "y": -130},
    {"name": "PC3", "template": "Alpine Linux", "x": -600, "y": -60},
    {"name": "PC4", "template": "Alpine Linux", "x": -600, "y": 10},
    {"name": "PC5", "template": "Alpine Linux", "x": -600, "y": 80},
    {"name": "PC6", "template": "Alpine Linux", "x": -600, "y": 150},
    {"name": "PC7", "template": "Alpine Linux", "x": -600, "y": 220},
    {"name": "SW1", "template": "Open vSwitch", "x": -350, "y": 10},
    {"name": "SW2", "template": "Open vSwitch", "x": -100, "y": 10},
    {"name": "R1", "template": "VyOS", "x": 150, "y": 10},
]
...
def main():
    ...
    print(
        f"\nDay 18 topology built in project '{PROJECT_NAME}'. "
