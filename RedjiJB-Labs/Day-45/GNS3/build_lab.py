#!/usr/bin/env python3
"""
GNS3 automation script for Day 45 - Dynamic NAT & PAT.

Builds a private-LAN-to-internet topology to practice pool-based Dynamic
NAT and interface-based PAT (NAT Overload), using free, open-source images:

    Role                    Image
    ----------------------  ----------------------------
    R1 (NAT router)         VyOS
    Internet Router         VyOS
    PC1, PC2, PC3           Alpine Linux
    Server (8.8.8.8 stand-in)  Alpine Linux

Note: the original lab has no distinct firewall/ASA device - R1 itself
performs NAT - so no pfSense node is included here.
...
PROJECT_NAME = "Day-45-Dynamic-NAT-PAT"

REQUIRED_TEMPLATES = {
    "VyOS": {
        "download_url": "https://s3.amazonaws.com/s3-us.vyos.io/rolling/current/vyos-rolling-latest.iso",
        "note": "Router role (R1 - NAT router, Internet Router). VyOS supports both pool-based source NAT and `masquerade` (its PAT equivalent).",
    },
    "Alpine Linux": {
        "download_url": "https://dl-cdn.alpinelinux.org/alpine/latest-stable/releases/x86_64/",
        "note": "PC1, PC2, PC3 (internal hosts), Server (external test destination).",
    },
    "Open vSwitch": {
        "download_url": None,
        "note": "Optional switch grouping the 3 PCs. Built into GNS3 - no image download needed.",
    },
}

NODES = [
    {"name": "PC1", "template": "Alpine Linux", "x": -400, "y": -100},
    {"name": "PC2", "template": "Alpine Linux", "x": -400, "y": 0},
    {"name": "PC3", "template": "Alpine Linux", "x": -400, "y": 100},
    {"name": "SW1", "template": "Open vSwitch", "x": -250, "y": 0},
    {"name": "R1", "template": "VyOS", "x": -100, "y": 0},
    {"name": "InternetRouter", "template": "VyOS", "x": 100, "y": 0},
    {"name": "Server", "template": "Alpine Linux", "x": 250, "y": 0},
]
...
def main():
    ...
    print(
        f"\nDay 45 topology built in project '{PROJECT_NAME}'. "
        "Open it in the GNS3 GUI, address all interfaces, and configure "
        "Dynamic NAT on R1 first (VyOS: `set nat source rule 10 "
        "translation address 100.0.0.1-100.0.0.2`), test pool exhaustion "
        "with all 3 PCs, then switch to PAT (VyOS: `set nat source rule 10 "
        "translation address masquerade`) and re-test to confirm all 3 "
        "PCs succeed."
    )
