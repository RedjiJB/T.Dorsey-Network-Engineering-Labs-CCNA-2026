#!/usr/bin/env python3
"""
GNS3 automation script for Day 43 - FTP & TFTP: Cisco IOS File Transfer &
Upgrade.

Builds a router pair connected by a /30 link, with a file server LAN, using
free, open-source images:
    ...
    Server (SRV1)       Alpine Linux (install tftp-hpa and vsftpd)
...
PROJECT_NAME = "Day-43-FTP-TFTP-IOS-Transfer"

REQUIRED_TEMPLATES = {
    "VyOS": {
        "download_url": "https://s3.amazonaws.com/s3-us.vyos.io/rolling/current/vyos-rolling-latest.iso",
        "note": "Router role (R1, R2). Practice `copy tftp:`/`copy ftp:`-equivalent file operations (VyOS uses `add system image` for actual OS upgrades, but standard `curl`/`tftp` client commands work fine for generic file transfer practice).",
    },
    "Alpine Linux": {
        "download_url": "https://dl-cdn.alpinelinux.org/alpine/latest-stable/releases/x86_64/",
        "note": "SRV1 - install tftp-hpa (TFTP server) and vsftpd (FTP server) to serve files to the routers.",
    },
    "Open vSwitch": {
        "download_url": None,
...
NODES = [
    {"name": "SRV1", "template": "Alpine Linux", "x": -400, "y": 0},
    {"name": "SW1", "template": "Open vSwitch", "x": -250, "y": 0},
    {"name": "R1", "template": "VyOS", "x": -100, "y": 0},
    {"name": "R2", "template": "VyOS", "x": 100, "y": 0},
]
...
def main():
    ...
    print(
        f"\nDay 43 topology built in project '{PROJECT_NAME}'. "
        "Open it in the GNS3 GUI, address the interfaces and configure a "
        "static route on R2, install tftp-hpa and vsftpd on SRV1's Alpine "
        "shell, place a sample file to transfer, and practice `tftp -g` / "
        "`curl ftp://` (or VyOS's own file-copy operators) from R1 and R2 "
        "against SRV1."
    )
