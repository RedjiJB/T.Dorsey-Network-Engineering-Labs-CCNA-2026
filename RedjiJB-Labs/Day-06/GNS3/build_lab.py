#!/usr/bin/env python3
"""
GNS3 automation script for Day 06 - Ethernet LAN Switching & MAC Address Tables.

Builds the two-switch, four-PC flat LAN from
Labs/Day-06-Ethernet-LAN-Switching-1.md using free, open-source images:

    Role            Image
    --------------  ----------------------------
    Switches        Open vSwitch (GNS3 built-in)
    PC1-PC4         Linux (Alpine)
...
NODES = [
    {"name": "PC1", "template": "Alpine Linux", "x": -300, "y": -100},
    {"name": "PC2", "template": "Alpine Linux", "x": -300, "y": 0},
    {"name": "SW1", "template": "Open vSwitch", "x": -100, "y": -50},
    {"name": "SW2", "template": "Open vSwitch", "x": 100, "y": -50},
    {"name": "PC3", "template": "Alpine Linux", "x": 300, "y": -100},
    {"name": "PC4", "template": "Alpine Linux", "x": 300, "y": 0},
]
...
def main():
    ...
    print(
        f"\nDay 06 topology built in project '{PROJECT_NAME}'. "
        "Open it in the GNS3 GUI to start the nodes. Use "
        "'ovs-appctl fdb/show <bridge>' on the Open vSwitch nodes (via the "
        "GNS3 host or an attached console) instead of 'show mac "
        "address-table', and use Wireshark on a link to observe the "
        "ARP-flood-then-unicast pattern described in the companion manual."
    )
