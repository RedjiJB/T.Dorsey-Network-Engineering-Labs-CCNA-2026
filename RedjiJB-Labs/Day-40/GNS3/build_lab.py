#!/usr/bin/env python3
"""
GNS3 automation script for Day 40 - SNMP Fundamentals, MIB Queries, and
Remote Device Management.

Builds a single-router SNMP agent/manager topology using free, open-source
images:
    ...
    Manager (PC1)       Alpine Linux (install net-snmp-tools for
...
PROJECT_NAME = "Day-40-SNMP-Fundamentals"

REQUIRED_TEMPLATES = {
    "VyOS": {
        "download_url": "https://s3.amazonaws.com/s3-us.vyos.io/rolling/current/vyos-rolling-latest.iso",
        "note": "Router role (R1), acts as SNMP agent (VyOS supports `set service snmp`).",
    },
    "Alpine Linux": {
        "download_url": "https://dl-cdn.alpinelinux.org/alpine/latest-stable/releases/x86_64/",
        "note": "Management workstation (PC1), SNMP manager - install net-snmp-tools.",
    },
    "Open vSwitch": {
        "download_url": None,
...
def main():
    ...
    print(
        f"\nDay 40 topology built in project '{PROJECT_NAME}'. "
        "Open it in the GNS3 GUI, configure SNMP on R1 (VyOS: `set service "
        "snmp community <string> authorization ro|rw`), install net-snmp-tools "
        "on PC1's Alpine shell, and run snmpget/snmpwalk/snmpset against R1."
    )
