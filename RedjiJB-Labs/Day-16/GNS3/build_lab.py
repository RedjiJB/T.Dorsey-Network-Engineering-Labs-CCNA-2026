#!/usr/bin/env python3
"""
GNS3 automation script for Day 16 - VLANs Part 1: Configuration and Inter-VLAN Routing.

Builds the SW1/R1 VLAN topology from RedjiJB-Labs/Day-16/Day-16-Lab-Manual.md
...
# Three separate physical links from R1 to SW1, one per VLAN -- no trunking.
NODES = [
    {"name": "PC1", "template": "Alpine Linux", "x": -500, "y": -200},
    {"name": "PC2", "template": "Alpine Linux", "x": -500, "y": -120},
    {"name": "PC3", "template": "Alpine Linux", "x": -500, "y": -40},
    {"name": "PC4", "template": "Alpine Linux", "x": -500, "y": 40},
    {"name": "PC5", "template": "Alpine Linux", "x": -500, "y": 120},
    {"name": "PC6", "template": "Alpine Linux", "x": -500, "y": 200},
    {"name": "SW1", "template": "Open vSwitch", "x": -250, "y": 0},
    {"name": "R1", "template": "VyOS", "x": 0, "y": 0},
]
...
LINKS = [
    ...
    ("PC5", "SW1"),
    ("PC6", "SW1"),
    ("SW1", "R1"),  # VLAN10 uplink
    ("SW1", "R1"),  # VLAN20 uplink
    ("SW1", "R1"),  # VLAN30 uplink
]
...
def create_links(project_id, node_ids):
    """
    Creates links in LINKS order. Because R1<->SW1 appears three times (one
    per VLAN), port tracking per-node is essential so each of the three
    links lands on a distinct port on both R1 and SW1 rather than
    colliding on port 0.
    """
    existing_links = gns3_get(f"/projects/{project_id}/links")
    next_port = {}  # node_id -> next free port_number
    link_count = {}  # (node_a, node_b) pair -> how many links already made
    ...
    for a, b in LINKS:
        ...
        pair = (node_a, node_b)
        link_count[pair] = link_count.get(pair, 0) + 1
...
def main():
    ...
    print(
        f"\nDay 16 topology built in project '{PROJECT_NAME}'. "
        "Open it in the GNS3 GUI to start the nodes.\n"
        "Note: this script does not re-run link creation idempotently for "
        "the repeated R1<->SW1 pair (unlike other Day scripts) because "
        "three parallel links between the same two nodes is intentional "
        "here -- re-running this script will add duplicate links. Delete "
        "the project and re-run if you need a clean rebuild."
    )
