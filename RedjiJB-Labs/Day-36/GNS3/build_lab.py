#!/usr/bin/env python3
"""
GNS3 automation script for Day 36 - CDP & LLDP: Network Discovery Protocols.

Builds a three-router triangle, each with its own access switch and PC, using
free, open-source images:

    Role                  Image
    --------------------  ----------------------------
    Routers (R1, R2, R3)  VyOS
    Switches (SW1-3)      Open vSwitch
    PCs (PC1-3)           Alpine Linux

Note: VyOS does not run Cisco's proprietary CDP. This lab's GNS3 build is
intended for practicing the LLDP portion (Phase 4) hands-on -- VyOS supports
LLDP natively. Phases 1-3 (CDP discovery and disable) are Cisco-IOS-specific
and are best practiced in Packet Tracer per the lab manual; this topology
still mirrors that same physical layout for consistency.
...
PROJECT_NAME = "Day-36-CDP-LLDP"

REQUIRED_TEMPLATES = {
    "VyOS": {
        "download_url": "https://s3.amazonaws.com/s3-us.vyos.io/rolling/current/vyos-rolling-latest.iso",
        "note": "Router role (R1, R2, R3). Supports LLDP natively (no CDP -- Cisco proprietary).",
    },
    "Alpine Linux": {
        "download_url": "https://dl-cdn.alpinelinux.org/alpine/latest-stable/releases/x86_64/",
        "note": "End devices (PC1-3).",
    },
    "Open vSwitch": {
        "download_url": None,
        "note": "Access switches (SW1-3). Built into GNS3.",
    },
}

NODES = [
    {"name": "R1", "template": "VyOS", "x": -200, "y": -200},
    {"name": "R2", "template": "VyOS", "x": 0, "y": 0},
    {"name": "R3", "template": "VyOS", "x": -200, "y": 200},
    {"name": "SW1", "template": "Open vSwitch", "x": -450, "y": -200},
    {"name": "SW2", "template": "Open vSwitch", "x": 250, "y": 0},
    {"name": "SW3", "template": "Open vSwitch", "x": -450, "y": 200},
    {"name": "PC1", "template": "Alpine Linux", "x": -650, "y": -200},
    {"name": "PC2", "template": "Alpine Linux", "x": 450, "y": 0},
    {"name": "PC3", "template": "Alpine Linux", "x": -650, "y": 200},
]
...
def main():
    ...
    print(
        f"\nDay 36 topology built in project '{PROJECT_NAME}'. "
        "Open it in the GNS3 GUI to start the nodes, then enable LLDP on "
        "each VyOS router ('set service lldp interface all') and restrict "
        "it on each switch's PC-facing port per the lab manual's Phase 4."
    )
