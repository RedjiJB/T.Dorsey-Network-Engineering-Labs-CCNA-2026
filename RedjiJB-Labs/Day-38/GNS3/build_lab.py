#!/usr/bin/env python3
"""
GNS3 automation script for Day 38 - DNS Configuration and Name Resolution.

Builds an internal LAN (R1 + three clients) with a default route toward a
simulated internet containing a DNS server and a web server, using free,
open-source images:

    Role                          Image
    --------------------------    ----------------------------
    Routers (R1, Internet Router) VyOS
    Switch (SW1)                  Open vSwitch (GNS3 built-in)
    PCs (PC1, PC2, PC3)           Linux (Alpine)
    DNS Server, Web Server        Linux (Alpine) running dnsmasq / a web server
...
PROJECT_NAME = "Day-38-DNS-Configuration"

REQUIRED_TEMPLATES = {
    "VyOS": {
        "download_url": "https://s3.amazonaws.com/s3-us.vyos.io/rolling/current/vyos-rolling-latest.iso",
        "note": "Router role (R1, Internet Router).",
    },
    "Alpine Linux": {
        "download_url": "https://dl-cdn.alpinelinux.org/alpine/latest-stable/releases/x86_64/",
        "note": "PC1-PC3, plus DNS Server (dnsmasq) and Web Server nodes.",
    },
    "Open vSwitch": {
        "download_url": None,
        "note": "Internal LAN switch role (SW1). Built into GNS3 — no image download needed.",
    },
}

NODES = [
    {"name": "PC1", "template": "Alpine Linux", "x": -500, "y": -150},
    {"name": "PC2", "template": "Alpine Linux", "x": -500, "y": 0},
    {"name": "PC3", "template": "Alpine Linux", "x": -500, "y": 150},
    {"name": "SW1", "template": "Open vSwitch", "x": -300, "y": 0},
    {"name": "R1", "template": "VyOS", "x": -100, "y": 0},
    {"name": "Internet_Router", "template": "VyOS", "x": 100, "y": 0},
    {"name": "DNS_Server", "template": "Alpine Linux", "x": 300, "y": -100},
    {"name": "Web_Server", "template": "Alpine Linux", "x": 300, "y": 100},
]
...
LINKS = [
    ...
    ("SW1", "R1"),
    ("R1", "Internet_Router"),
    ("Internet_Router", "DNS_Server"),
    ("Internet_Router", "Web_Server"),
]
...
def main():
    ...
    print(
        f"\nDay 38 topology built in project '{PROJECT_NAME}'. "
        "Open it in the GNS3 GUI to start the nodes. See README.md for the "
        "VyOS default-route/DNS configuration and the dnsmasq setup on the "
        "Alpine DNS_Server node."
    )
