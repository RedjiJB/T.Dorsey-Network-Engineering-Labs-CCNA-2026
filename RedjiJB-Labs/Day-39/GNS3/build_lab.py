#!/usr/bin/env python3
"""
GNS3 automation script for Day 39 - DHCP Server, DHCP Client, and DHCP Relay.

Builds the two-site topology from Labs/Day-39-...DHCP-Relay.md using free,
open-source images:
...
PROJECT_NAME = "Day-39-DHCP-Server-Client-Relay"

REQUIRED_TEMPLATES = {
    "VyOS": {
        "download_url": "https://s3.amazonaws.com/s3-us.vyos.io/rolling/current/vyos-rolling-latest.iso",
        "note": "Router role (R1, R2). Open-source, Cisco-like CLI, supports DHCP server/client/relay.",
    },
    "Alpine Linux": {
        "download_url": "https://dl-cdn.alpinelinux.org/alpine/latest-stable/releases/x86_64/",
        "note": "End devices (PC1, PC2). Lightweight full Linux, supports `udhcpc` for DHCP client testing.",
    },
    "Open vSwitch": {
        "download_url": None,
...
NODES = [
    ...
    {"name": "SW2", "template": "Open vSwitch", "x": 250, "y": 150},
...
def main():
    ...
    print(
        f"\nDay 39 topology built in project '{PROJECT_NAME}'. "
        "Open it in the GNS3 GUI, then configure R2 as the DHCP server "
        "(three pools), R1's WAN-facing interface as a DHCP client, and "
        "R1's LAN-facing interface with an ip helper-address / VyOS "
        "equivalent DHCP relay pointing at R2."
    )
