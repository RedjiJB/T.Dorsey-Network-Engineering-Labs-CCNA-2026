#!/usr/bin/env python3
"""
GNS3 automation script for Day 20 - Analyzing STP: Port Roles Across Four
Switches.

Builds a redundant four-switch mesh topology using free, open-source images:

    Role                            Image
    ------------------------------  ----------------------------
    Switches (SW1, SW2, SW3, SW4)   Open vSwitch (GNS3 built-in) --
                                     see README for STP support caveats
...
PROJECT_NAME = "Day-20-Analyzing-STP-Port-Roles"

REQUIRED_TEMPLATES = {
    "Open vSwitch": {
        "download_url": None,
        "note": "Switch role (SW1-SW4). Built into GNS3 -- no image "
        "download needed. Limited STP support -- see README.",
    },
}

# A redundant mesh: SW3 is the intended root (lowest priority in the
# manual's topology), directly linked to all three others, which are
# also cross-linked to each other -- creating the alternate/blocking
# ports this lab analyzes.
NODES = [
    {"name": "SW3", "template": "Open vSwitch", "x": 0, "y": -150},
    {"name": "SW1", "template": "Open vSwitch", "x": -200, "y": 0},
    {"name": "SW2", "template": "Open vSwitch", "x": 0, "y": 150},
    {"name": "SW4", "template": "Open vSwitch", "x": 200, "y": 0},
]
...
def check_templates():
    ...
    if not missing:
        ...
        print(
            "\nREMINDER: Open vSwitch has limited STP support. See "
            "GNS3/README.md for the Cisco IOSvL2/vIOS-L2 alternative if "
            "you need accurate `show spanning-tree detail` output.\n"
        )
        return
...
def main():
    ...
    print(
        f"\nDay 20 topology built in project '{PROJECT_NAME}'. "
