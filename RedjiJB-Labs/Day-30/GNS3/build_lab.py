#!/usr/bin/env python3
"""
GNS3 automation script for Day 30 - HSRP Gateway Redundancy: Failover,
Preemption, and Virtual IPs.

Builds a redundant-gateway LAN topology (R1 + R2 sharing a first-hop
redundancy virtual IP, R3 as the WAN/ISP edge, plus two end hosts) using
free, open-source images:

    Role                Image
    ------------------  ----------------------------
    Routers (R1,R2,R3)  VyOS
    Switch (SW1)         Open vSwitch (GNS3 built-in)
    PCs (PC1, PC2)        Linux (Alpine)

IMPORTANT PROTOCOL NOTE: HSRP is a Cisco-proprietary protocol. VyOS (and
FRRouting underneath it) does not implement HSRP — it implements VRRP, the
open IETF standard equivalent. This build uses VRRP to demonstrate the same
first-hop-redundancy concept (virtual IP, priority election, preemption,
virtual MAC) with an open-source router image. Concept mapping:

    HSRP (Cisco, IOS)                  VRRP (open standard, VyOS)
    ----------------------------       ----------------------------
    standby <grp> ip <vip>             set high-availability vrrp group <name> virtual-address <vip>
    standby <grp> priority <n>         set high-availability vrrp group <name> priority <n>
    standby <grp> preempt              preemption is VRRP's default behavior (opt OUT with no-preempt)
...
NODES = [
    {"name": "PC1", "template": "Alpine Linux", "x": -400, "y": -100},
    {"name": "PC2", "template": "Alpine Linux", "x": -400, "y": 100},
    {"name": "SW1", "template": "Open vSwitch", "x": -200, "y": 0},
    {"name": "R1", "template": "VyOS", "x": 0, "y": -100},
    {"name": "R2", "template": "VyOS", "x": 0, "y": 100},
    {"name": "R3", "template": "VyOS", "x": 200, "y": 0},
]
...
def main():
    ...
    print(
        f"\nDay 30 topology built in project '{PROJECT_NAME}'. "
        "Open it in the GNS3 GUI to start the nodes. See README.md for the "
        "VyOS VRRP configuration (the open-standard equivalent to HSRP) to "
        "apply on R1 and R2 once they're running."
    )
