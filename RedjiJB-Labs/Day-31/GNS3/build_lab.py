#!/usr/bin/env python3
"""
GNS3 automation script for Day 31 - IPv6 Dual-Stack Configuration on a Multi-LAN Router.

Builds a single-router, three-LAN dual-stack topology using free, open-source images:

    Role                  Image
    --------------------  ----------------------------
    Router (R1)           VyOS
    Switches (SW1-SW3)    Open vSwitch (GNS3 built-in)
    PCs (PC1-PC3)         Alpine Linux
...
PROJECT_NAME = "Day-31-IPv6-Dual-Stack"

REQUIRED_TEMPLATES = {
    "VyOS": {
        "download_url": "https://s3.amazonaws.com/s3-us.vyos.io/rolling/current/vyos-rolling-latest.iso",
        "note": "Router role (R1). Open-source, Cisco-like CLI, supports IPv6.",
    },
    "Alpine Linux": {
        "download_url": "https://dl-cdn.alpinelinux.org/alpine/latest-stable/releases/x86_64/",
        "note": "End devices (PC1-PC3). Lightweight full Linux, supports IPv6 natively.",
    },
    "Open vSwitch": {
        "download_url": None,
        "note": "Switch role (SW1-SW3). Built into GNS3 - no image download needed.",
    },
}

NODES = [
    {"name": "R1", "template": "VyOS", "x": 0, "y": -150},
    {"name": "SW1", "template": "Open vSwitch", "x": -300, "y": -250},
    {"name": "SW2", "template": "Open vSwitch", "x": -300, "y": -150},
    {"name": "SW3", "template": "Open vSwitch", "x": -300, "y": -50},
    {"name": "PC1", "template": "Alpine Linux", "x": -500, "y": -250},
...
def main():
    ...
    print(
        f"\nDay 31 topology built in project '{PROJECT_NAME}'. "
