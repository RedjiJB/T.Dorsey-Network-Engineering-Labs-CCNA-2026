#!/usr/bin/env python3
"""
GNS3 automation script for Day 37 - NTP: Network Time Synchronization.

Builds a three-router full-mesh topology plus a simulated external NTP
reference host, using free, open-source images:

    Role                        Image
    --------------------------  ----------------------------
    Routers (R1, R2, R3)        VyOS
    External NTP host (NTP-REF) Alpine Linux (runs chrony/ntpd as a stratum
                                 1-like reference for the lab)
...
PROJECT_NAME = "Day-37-NTP"

REQUIRED_TEMPLATES = {
    "VyOS": {
        "download_url": "https://s3.amazonaws.com/s3-us.vyos.io/rolling/current/vyos-rolling-latest.iso",
        "note": "Router role (R1, R2, R3). Supports NTP client/server/authentication natively.",
    },
    "Alpine Linux": {
        "download_url": "https://dl-cdn.alpinelinux.org/alpine/latest-stable/releases/x86_64/",
        "note": "NTP-REF -- stand-in for the external stratum 1 server (1.1.1.1 in the lab manual). Runs chrony as a local reference clock.",
    },
}

NODES = [
    {"name": "R1", "template": "VyOS", "x": -100, "y": 0},
    {"name": "R2", "template": "VyOS", "x": 100, "y": -150},
    {"name": "R3", "template": "VyOS", "x": 100, "y": 150},
    {"name": "NTP-REF", "template": "Alpine Linux", "x": -350, "y": 0},
]
...
def main():
    ...
    print(
        f"\nDay 37 topology built in project '{PROJECT_NAME}'. "
        "Open it in the GNS3 GUI to start the nodes, then configure NTP-REF "
        "as a local chrony reference clock, R1 as its client (then master "
        "for R2/R3 with authentication) per the lab manual and README."
    )
