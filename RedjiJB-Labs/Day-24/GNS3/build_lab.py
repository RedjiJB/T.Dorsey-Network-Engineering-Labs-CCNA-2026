#!/usr/bin/env python3
"""
GNS3 automation script for Day 24 - Floating Static Routes and Failover Testing.

Builds the R1/R2/ISPBR1/ISPBR2 dual-homed topology from
Labs/Day-24-Floating-Static-Routes-and-Failover-Testing.md using free,
open-source images:

    Role                        Image
    --------------------------  ----------------------------
    Routers (R1, R2, ISPBR1/2)  VyOS
    Switches (SW1, SW2)         Open vSwitch (GNS3 built-in)
    PC1, SRV1                   Alpine Linux
...
PROJECT_NAME = "Day-24-Floating-Static-Routes"

REQUIRED_TEMPLATES = {
    "VyOS": {
        "download_url": "https://s3.amazonaws.com/s3-us.vyos.io/rolling/current/vyos-rolling-latest.iso",
        "note": "Router role (R1, R2, ISPBR1, ISPBR2). Open-source, Cisco-like CLI.",
    },
    "Open vSwitch": {
        "download_url": None,
    ...
    "Alpine Linux": {
        ...
        "note": "End devices (PC1, SRV1). Lightweight full Linux.",
    },
}
...
NODES = [
    ...
    {"name": "ISPBR1", "template": "VyOS", "x": -50, "y": -200},
    {"name": "R2", "template": "VyOS", "x": 100, "y": 0},
    {"name": "ISPBR2", "template": "VyOS", "x": -50, "y": 100},
...
LINKS = [
    ...
    ("SW1", "R1"),
    ("R1", "R2"),          # OSPF backbone
    ("R1", "ISPBR1"),      # R1 to its ISP border router
    ("R2", "ISPBR2"),      # R2 to its ISP border router
    ("ISPBR1", "ISPBR2"),  # ISP cloud link, carries reciprocal floating statics
    ("R2", "SW2"),
    ("SW2", "SRV1"),
]
...
def main():
    ...
    print(
        f"\nDay 24 topology built in project '{PROJECT_NAME}'. "
