#!/usr/bin/env python3
"""
GNS3 automation script for Day 42 - SSH: Secure Remote Access & Management.

Builds a two-LAN topology connected by a routed WAN link, with a switch that
must be hardened for SSH-only, ACL-restricted management access, using free,
open-source images:
...
PROJECT_NAME = "Day-42-SSH-Secure-Remote-Access"

REQUIRED_TEMPLATES = {
    "VyOS": {
        "download_url": "https://s3.amazonaws.com/s3-us.vyos.io/rolling/current/vyos-rolling-latest.iso",
        "note": "Router role (R1, R2), connects the two LANs.",
    },
    "Alpine Linux": {
        "download_url": "https://dl-cdn.alpinelinux.org/alpine/latest-stable/releases/x86_64/",
        "note": "PC1 (SSH client, install openssh-client), Laptop1 (console-equivalent host).",
    },
    "Open vSwitch": {
        "download_url": None,
        "note": "Switch role (SW1, SW2, the device being hardened is SW2). Built into GNS3 - no image download needed.",
    },
}

NODES = [
    {"name": "PC1", "template": "Alpine Linux", "x": -400, "y": -100},
    {"name": "SW1", "template": "Open vSwitch", "x": -250, "y": -100},
    {"name": "R1", "template": "VyOS", "x": -100, "y": -100},
    {"name": "R2", "template": "VyOS", "x": 100, "y": 100},
    {"name": "SW2", "template": "Open vSwitch", "x": 250, "y": 100},
    {"name": "Laptop1", "template": "Alpine Linux", "x": 400, "y": 100},
]
...
def main():
    ...
    print(
        f"\nDay 42 topology built in project '{PROJECT_NAME}'. "
        "Open it in the GNS3 GUI, configure static routing between R1 and "
        "R2, harden SW2 (VyOS equivalent: `set service ssh` + `set "
        "firewall` rules restricting management access to PC1's address), "
        "install openssh-client on PC1's Alpine shell (`apk add openssh-"
        "client`), and test `ssh jeremy@<SW2-mgmt-ip>`."
    )
