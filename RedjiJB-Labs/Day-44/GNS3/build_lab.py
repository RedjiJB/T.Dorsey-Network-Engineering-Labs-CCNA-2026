#!/usr/bin/env python3
"""
GNS3 automation script for Day 44 - Static NAT.

Builds a private-LAN-to-internet topology with a NAT router, using free,
open-source images:
...
PROJECT_NAME = "Day-44-Static-NAT"

REQUIRED_TEMPLATES = {
    "VyOS": {
        "download_url": "https://s3.amazonaws.com/s3-us.vyos.io/rolling/current/vyos-rolling-latest.iso",
        "note": "Router role (R1 - NAT router, Internet Router). VyOS supports `set nat source rule` for static one-to-one NAT.",
    },
    "Alpine Linux": {
    ...
    "Open vSwitch": {
        "download_url": None,
        "note": "Optional - add if you want a dedicated switch between the PCs and R1 instead of direct links.",
    },
}
...
def main():
    ...
    print(
        f"\nDay 44 topology built in project '{PROJECT_NAME}'. "
        "Open it in the GNS3 GUI, address all interfaces, and configure "
        "static NAT on R1 (VyOS equivalent: `set nat source rule 10 "
        "source address 172.16.0.1`, `set nat source rule 10 translation "
        "address 100.0.0.1`, plus `set nat source rule 10 outbound-"
        "interface <wan-if>`), repeated per PC. Test ping from each PC to "
        "the Server node before and after configuring NAT."
    )
