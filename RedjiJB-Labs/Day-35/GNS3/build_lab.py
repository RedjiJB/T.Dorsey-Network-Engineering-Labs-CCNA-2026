#!/usr/bin/env python3
"""
GNS3 automation script for Day 35 - Extended ACLs: Destination and Port-Based Filtering.

This lab reuses Day 34's exact topology (two routers, four subnets) unchanged --
Day 35 is a policy upgrade (standard ACLs -> extended ACLs), not a new network
build. This script builds the identical topology under its own project name so
Day 34 and Day 35 can be worked independently in GNS3.

    Role                  Image
    --------------------  ----------------------------
    Routers (R1, R2)      VyOS
    PCs/Servers           Alpine Linux
    LAN switches          Open vSwitch (built into GNS3)
...
PROJECT_NAME = "Day-35-Extended-ACLs"

REQUIRED_TEMPLATES = {
    "VyOS": {
        "download_url": "https://s3.amazonaws.com/s3-us.vyos.io/rolling/current/vyos-rolling-latest.iso",
        "note": "Router role (R1, R2). Supports OSPF and firewall rule-sets with protocol/port match criteria (extended ACL equivalent).",
    },
    "Alpine Linux": {
        "download_url": "https://dl-cdn.alpinelinux.org/alpine/latest-stable/releases/x86_64/",
        "note": "End devices (PC1-4, SRV1-2).",
    },
    "Open vSwitch": {
        "download_url": None,
        "note": "Used to fan out each multi-host LAN segment. Built into GNS3.",
    },
}

NODES = [
    {"name": "PC1", "template": "Alpine Linux", "x": -600, "y": -250},
    {"name": "PC2", "template": "Alpine Linux", "x": -600, "y": -150},
    {"name": "SW-LAN1", "template": "Open vSwitch", "x": -450, "y": -200},
    {"name": "PC3", "template": "Alpine Linux", "x": -600, "y": -50},
    {"name": "PC4", "template": "Alpine Linux", "x": -600, "y": 50},
    {"name": "SW-LAN2", "template": "Open vSwitch", "x": -450, "y": 0},
    {"name": "R1", "template": "VyOS", "x": -250, "y": -100},
    {"name": "R2", "template": "VyOS", "x": 50, "y": -100},
    {"name": "SW-SRV1", "template": "Open vSwitch", "x": 250, "y": -200},
    {"name": "SRV1", "template": "Alpine Linux", "x": 400, "y": -200},
    {"name": "SW-SRV2", "template": "Open vSwitch", "x": 250, "y": 0},
    {"name": "SRV2", "template": "Alpine Linux", "x": 400, "y": 0},
]
...
def main():
    ...
    print(
        f"\nDay 35 topology built in project '{PROJECT_NAME}'. "
        "Open it in the GNS3 GUI to start the nodes, then configure R1's "
        "VyOS firewall rule-sets to mirror this lab's three extended ACL "
        "policies (protocol + destination + port match criteria)."
    )
