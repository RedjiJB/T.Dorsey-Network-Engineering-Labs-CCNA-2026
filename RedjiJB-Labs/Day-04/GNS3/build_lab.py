#!/usr/bin/env python3
"""
GNS3 automation script for Day 04 - Basic Device Security & Cisco IOS Administration.

Builds the single router/switch hardening topology from
Labs/Day-04-Basic-Device-Security.md using free, open-source images:
...
PROJECT_NAME = "Day-04-Basic-Device-Security"
...
def main():
    ...
    print(
        f"\nDay 04 topology built in project '{PROJECT_NAME}'. "
        "Open it in the GNS3 GUI to start the nodes. Apply VyOS's own "
        "hardening syntax (hashed local user passwords, 'set service ssh', "
        "disabling telnet) to R1 to practice the underlying concepts from "
        "the companion manual, even though exact command syntax differs "
        "from Cisco IOS."
    )
