#!/usr/bin/env python3
"""
GNS3 automation script for Day 08 - IPv4 Address Configuration & Router Interface Setup.

Builds the single-router, three-network topology from
Labs/Day-08-Lab-IPv4-Addresses-6.md using free, open-source images:

    Role                Image
    ------------------  ----------------------------
    Router (R1)         VyOS
    Switches (SW1-3)    Open vSwitch (GNS3 built-in)
    PC1-PC3             Linux (Alpine)
...
PROJECT_NAME = "Day-08-IPv4-Addresses"
...
REQUIRED_TEMPLATES = {
    ...
    "Alpine Linux": {
        ...
        "note": "End devices (PC1, PC2, PC3).",
    },
    "Open vSwitch": {
        "download_url": None,
        "note": "Switch role (SW1-3). Built into GNS3 - no image download needed.",
    },
}

NODES = [
    {"name": "PC1", "template": "Alpine Linux", "x": -300, "y": -150},
    {"name": "SW1", "template": "Open vSwitch", "x": -150, "y": -150},
    {"name": "PC2", "template": "Alpine Linux", "x": -300, "y": 0},
    {"name": "SW2", "template": "Open vSwitch", "x": -150, "y": 0},
    {"name": "PC3", "template": "Alpine Linux", "x": -300, "y": 150},
    {"name": "SW3", "template": "Open vSwitch", "x": -150, "y": 150},
    {"name": "R1", "template": "VyOS", "x": 50, "y": 0},
]
...
def main():
    ...
    print(
        f"\nDay 08 topology built in project '{PROJECT_NAME}'. "
        "Open it in the GNS3 GUI to start the nodes. Configure R1's three "
        "interfaces (VyOS: 'set interfaces ethernet ethN address <ip>/<prefix>') "
        "using CIDR notation directly - unlike Cisco IOS, VyOS accepts the "
        "slash prefix form, a useful contrast to note against the companion "
        "manual's IOS 'ip address <ip> <mask>' syntax."
    )
