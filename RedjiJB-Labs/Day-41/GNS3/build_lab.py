#!/usr/bin/env python3
"""
GNS3 automation script for Day 41 - Syslog Configuration, Logging
Destinations, and Remote Device Monitoring.

Builds a router + console host + Telnet client + Syslog server topology
...
PROJECT_NAME = "Day-41-Syslog-Logging"

REQUIRED_TEMPLATES = {
    "VyOS": {
        "download_url": "https://s3.amazonaws.com/s3-us.vyos.io/rolling/current/vyos-rolling-latest.iso",
        "note": "Router role (R1), generates and forwards Syslog messages.",
    },
    "Alpine Linux": {
        "download_url": "https://dl-cdn.alpinelinux.org/alpine/latest-stable/releases/x86_64/",
        "note": "PC1 (Telnet client), PC2 (console-equivalent host), SRV1 (install rsyslog for centralized logging).",
    },
    "Open vSwitch": {
        "download_url": None,
...
NODES = [
    {"name": "PC2", "template": "Alpine Linux", "x": -300, "y": -150},
    {"name": "PC1", "template": "Alpine Linux", "x": -300, "y": 0},
    {"name": "SW1", "template": "Open vSwitch", "x": -100, "y": 0},
    {"name": "R1", "template": "VyOS", "x": 100, "y": 0},
    {"name": "SRV1", "template": "Alpine Linux", "x": -300, "y": 150},
]

LINKS = [
    ("PC2", "R1"),   # console-equivalent management link
    ("PC1", "SW1"),
    ("SW1", "R1"),
    ("SRV1", "SW1"),
]
...
def main():
    ...
    print(
        f"\nDay 41 topology built in project '{PROJECT_NAME}'. "
        "Open it in the GNS3 GUI. Configure R1's logging (VyOS: `set system "
        "syslog host <SRV1-ip> facility all level debug`), install and "
        "start rsyslog on SRV1's Alpine shell (`apk add rsyslog && rc-service "
        "rsyslog start`), then Telnet/SSH from PC1 and test terminal monitor "
        "behavior."
    )
