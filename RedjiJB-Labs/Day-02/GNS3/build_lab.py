#!/usr/bin/env python3
"""
GNS3 automation script for Day 02 - Connecting Network Devices.

Builds the two-site (Site A / Site B) topology from
Labs/Day-02-Connecting-Devices.md using free, open-source images:

    Role                     Image
    -----------------------  ----------------------------
    Routers (R1-R4)          VyOS
    Switches (SW1-SW8)       Open vSwitch (GNS3 built-in)
    PCs / Server             Linux (Alpine)

Note: GNS3 virtual links do not model fiber vs. copper distance/medium.
The multi-mode/single-mode cable-selection exercise in the companion
manual is a Packet Tracer / design exercise; this script builds the
logical topology and addressing so you can practice routing/verification.
...
PROJECT_NAME = "Day-02-Connecting-Devices"

REQUIRED_TEMPLATES = {
    "VyOS": {
        "download_url": "https://s3.amazonaws.com/s3-us.vyos.io/rolling/current/vyos-rolling-latest.iso",
        "note": "Router role (R1-R4). Open-source, Cisco-like CLI.",
    },
    "Alpine Linux": {
        "download_url": "https://dl-cdn.alpinelinux.org/alpine/latest-stable/releases/x86_64/",
        "note": "End devices (PC1, PC2, PC3, SRV1). Lightweight full Linux.",
    },
    "Open vSwitch": {
        "download_url": None,
        "note": "Switch role (SW1-SW8). Built into GNS3 - no image download needed.",
    },
}
...
NODES = [
    ...
    {"name": "PC2", "template": "Alpine Linux", "x": -600, "y": -100},
    {"name": "SW3", "template": "Open vSwitch", "x": -450, "y": -200},
    {"name": "SW4", "template": "Open vSwitch", "x": -450, "y": -100},
    {"name": "SW1", "template": "Open vSwitch", "x": -300, "y": -200},
    {"name": "SW2", "template": "Open vSwitch", "x": -300, "y": -100},
    {"name": "R2", "template": "VyOS", "x": -150, "y": -150},
    {"name": "R1", "template": "VyOS", "x": 0, "y": -150},
    # Site B
    {"name": "R3", "template": "VyOS", "x": 300, "y": 150},
    {"name": "R4", "template": "VyOS", "x": 150, "y": 150},
    {"name": "SW5", "template": "Open vSwitch", "x": 0, "y": 150},
    {"name": "SW6", "template": "Open vSwitch", "x": 0, "y": 250},
    {"name": "SW7", "template": "Open vSwitch", "x": -150, "y": 150},
    {"name": "SW8", "template": "Open vSwitch", "x": -150, "y": 250},
    {"name": "PC3", "template": "Alpine Linux", "x": -300, "y": 150},
    {"name": "SRV1", "template": "Alpine Linux", "x": -300, "y": 250},
]
...
LINKS = [
    ...
    # Site A <-> hub router
    ("R2", "R1"),
    # Inter-site fiber link (logical link in GNS3; medium is a design concept, not modeled)
    ("R1", "R3"),
    # Site B
    ("R3", "R4"),
    ("R4", "SW5"),
...
def main():
    ...
    print(
        f"\nDay 02 topology built in project '{PROJECT_NAME}'. "
        "Open it in the GNS3 GUI to start the nodes. Remember: the "
        "copper/multi-mode/single-mode cable-selection exercise itself is "
        "a design exercise best done against the Packet Tracer topology or "
        "the reasoning in the companion manual, since GNS3 links don't "
        "model fiber distance."
    )
