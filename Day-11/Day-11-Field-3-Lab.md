# Day 11: VLAN Basics (Field 3: DePIN)

## 0. Metadata

- **Objective:** Prove VLAN Basics functionality in DePIN (Field 3) constraints
- **Research Field:** Field 3: DePIN
- **Proof Obligations:**
  - Validate VLAN Basics works without external dependencies (Field 1) / under stress (Field 2) / in mesh topology (Field 3) / at Haiti scale (Field 7)
...
- **Prerequisites:** Day 11 base lab manual + Field 3 environment setup
...
- **Key Concepts:** Field-optimized VLAN Basics, DePIN-specific validation, proof obligations, real-world constraints

## 1. Business Context

DePIN (Decentralized Physical Infrastructure Networks) rely on **mesh topologies** without central authority.
This lab proves VLAN Basics works in **full-mesh, Byzantine-fault-tolerant** design:
- No central hub or single point of failure
- Nodes must reach consensus on VLAN Basics without a leader
- Byzantine nodes (malicious or failed) must not break the network
- Voting quorum (n/2 + 1) must validate all state changes

**Why this matters:** Decentralized networks cannot rely on centralized control. VLAN Basics must be fault-tolerant at the core.
...
- Test VLAN Basics with Byzantine nodes (R4 offline or lying)
...
- Implement voting/consensus for VLAN Basics decisions
...
|--------|------|--------|---------|------|-------|
| R1 | Primary | 10.0.11.0/24 | 10.0.11.1 | /24 | Field 3 core router |
| R2 | Secondary | 10.0.11.0/24 | 10.0.11.2 | /24 | Backup/mesh peer |
| SW1 | Access | 10.0.11.0/24 | 10.0.11.254 | /24 | Access switch VLAN |
| PC1 | Client | 10.0.11.0/24 | 10.0.11.10 | /24 | Test client 1 |
| PC2 | Client | 10.0.11.0/24 | 10.0.11.20 | /24 | Test client 2 |

**Field 3 specific notes:**
- Subnets chosen to test Day 11 concepts in isolation
...
```
! Standard initial configuration for Day 11: VLAN Basics
Router> en
Router# conf t
...
Router(config-if)# ip address 10.0.11.1 255.255.255.0
Router(config-if)# no shutdown
Router(config-if)# exit

! Add connections to other mesh peers
Router(config)# int g0/1
Router(config-if)# ip address 10.0.11.11 255.255.255.0
Router(config-if)# no shutdown
Router(config-if)# exit

Router(config)# int g0/2
Router(config-if)# ip address 10.0.11.21 255.255.255.0
Router(config-if)# no shutdown
Router(config-if)# exit

! Implement voting/consensus (pseudo-code):
! if (consensus_check(VLAN Basics) >= n/2 + 1):
!     accept_update()
! else:
!     reject_update()
...
Router# show ip route
C 10.0.11.0/24 is directly connected, GigabitEthernet0/0
C 10.0.11.11/24 is directly connected, GigabitEthernet0/1
C 10.0.11.21/24 is directly connected, GigabitEthernet0/2
...
PC1# ping 10.0.11.10
Reply from 10.0.11.10: bytes=32 time=7ms
...
GigabitEthernet0/0         10.0.11.1    YES manual up                    up
GigabitEthernet0/1         10.0.11.11   YES manual up                    up
Serial0/0                  10.0.11.2    YES manual up                    up
```
...
C     10.0.11.0/24 is directly connected, GigabitEthernet0/0
C     10.0.11.11/24 is directly connected, GigabitEthernet0/1
C     10.0.11.2/24 is directly connected, Serial0/0
```

### 7.3 Ping Output (Normal and Under Stress)
```
PC1# ping 10.0.11.10 -c 5
Request sent from ICMP seq=1, timeout=20ms
Reply from 10.0.11.10: bytes=32 time=5ms TTL=63
Reply from 10.0.11.10: bytes=32 time=6ms TTL=63
Reply from 10.0.11.10: bytes=32 time=7ms TTL=63
Reply from 10.0.11.10: bytes=32 time=5ms TTL=63
Reply from 10.0.11.10: bytes=32 time=6ms TTL=63
...
**Why mesh topology for VLAN Basics?**
...
**This lab validates:** VLAN Basics works in Byzantine-fault-tolerant mesh
