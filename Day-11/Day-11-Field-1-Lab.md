# Day 11: VLAN Basics (Field 1: Black Start)

## 0. Metadata

- **Objective:** Prove VLAN Basics functionality in Black Start (Field 1) constraints
...
- **Prerequisites:** Day 11 base lab manual + Field 1 environment setup
...
- **Key Concepts:** Field-optimized VLAN Basics, Black Start-specific validation, proof obligations, real-world constraints

## 1. Business Context

Haiti P38 pilot sites experience frequent power losses (avg. 6 hours/day) and unreliable internet.
This lab validates that VLAN Basics works in **offline-only mode**:
...
**Why this matters:** Operators in remote areas cannot rely on central management. VLAN Basics must work autonomously.
...
- Test: Can VLAN Basics survive power loss? Is state recovered from cache?
...
- Test VLAN Basics in "dark mode" (no external dependencies)
...
|--------|------|--------|---------|------|-------|
| R1 | Primary | 10.0.11.0/24 | 10.0.11.1 | /24 | Field 1 core router |
...
! Static routes only
Router(config)# ip route 10.0.11.0 255.255.255.0 10.0.11.1
...
PC1# ping 10.0.11.10
Reply from 10.0.11.10: bytes=32 time=5ms
...
**Why offline-only topology for VLAN Basics?**
...
The offline-only design proves VLAN Basics works **without** these dependencies:
...
- Validates: VLAN Basics is truly offline-capable for Haiti deployment
...
4. VLAN Basics works autonomously without cloud sync
...
- Success: VLAN Basics validated for real Haiti deployment
