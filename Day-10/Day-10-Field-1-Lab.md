# Day 10: Hands-On Lab: IPv4 Fundamentals (Field 1: Black Start)

## 0. Metadata

- **Objective:** Prove Hands-On Lab: IPv4 Fundamentals functionality in Black Start (Field 1) constraints
- **Research Field:** Field 1: Black Start
- **Proof Obligations:**
  - Validate Hands-On Lab: IPv4 Fundamentals works without external dependencies (Field 1) / under stress (Field 2) / in mesh topology (Field 3) / at Haiti scale (Field 7)
...
- **Prerequisites:** Day 10 base lab manual + Field 1 environment setup
...
- **Key Concepts:** Field-optimized Hands-On Lab: IPv4 Fundamentals, Black Start-specific validation, proof obligations, real-world constraints

## 1. Business Context

Haiti P38 pilot sites experience frequent power losses (avg. 6 hours/day) and unreliable internet.
This lab validates that Hands-On Lab: IPv4 Fundamentals works in **offline-only mode**:
...
**Why this matters:** Operators in remote areas cannot rely on central management. Hands-On Lab: IPv4 Fundamentals must work autonomously.
...
- Test: Can Hands-On Lab: IPv4 Fundamentals survive power loss? Is state recovered from cache?
...
- Test Hands-On Lab: IPv4 Fundamentals in "dark mode" (no external dependencies)
...
|--------|------|--------|---------|------|-------|
| R1 | Primary | 10.0.10.0/24 | 10.0.10.1 | /24 | Field 1 core router |
| R2 | Secondary | 10.0.10.0/24 | 10.0.10.2 | /24 | Backup/mesh peer |
| SW1 | Access | 10.0.10.0/24 | 10.0.10.254 | /24 | Access switch VLAN |
| PC1 | Client | 10.0.10.0/24 | 10.0.10.10 | /24 | Test client 1 |
| PC2 | Client | 10.0.10.0/24 | 10.0.10.20 | /24 | Test client 2 |

**Field 1 specific notes:**
- Subnets chosen to test Day 10 concepts in isolation
...
```
! Standard initial configuration for Day 10: Hands-On Lab: IPv4 Fundamentals
Router> en
Router# conf t
...
! Static routes only
Router(config)# ip route 10.0.10.0 255.255.255.0 10.0.10.1
...
PC1# ping 10.0.10.10
Reply from 10.0.10.10: bytes=32 time=5ms
...
GigabitEthernet0/1         10.0.10.11   YES manual up                    up
Serial0/0                  10.0.10.2    YES manual up                    up
```
...
C     10.0.10.0/24 is directly connected, GigabitEthernet0/0
C     10.0.10.11/24 is directly connected, GigabitEthernet0/1
C     10.0.10.2/24 is directly connected, Serial0/0
```

### 7.3 Ping Output (Normal and Under Stress)
```
PC1# ping 10.0.10.10 -c 5
Request sent from ICMP seq=1, timeout=20ms
Reply from 10.0.10.10: bytes=32 time=5ms TTL=63
Reply from 10.0.10.10: bytes=32 time=6ms TTL=63
Reply from 10.0.10.10: bytes=32 time=7ms TTL=63
Reply from 10.0.10.10: bytes=32 time=5ms TTL=63
Reply from 10.0.10.10: bytes=32 time=6ms TTL=63
...
**Why offline-only topology for Hands-On Lab: IPv4 Fundamentals?**
...
The offline-only design proves Hands-On Lab: IPv4 Fundamentals works **without** these dependencies:
...
- Validates: Hands-On Lab: IPv4 Fundamentals is truly offline-capable for Haiti deployment
...
4. Hands-On Lab: IPv4 Fundamentals works autonomously without cloud sync
...
- Success: Hands-On Lab: IPv4 Fundamentals validated for real Haiti deployment
