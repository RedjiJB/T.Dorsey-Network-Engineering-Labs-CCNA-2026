# Day 09: Routing Protocols Intro (Field 1: Black Start)

## 0. Metadata

- **Objective:** Prove Routing Protocols Intro functionality in Black Start (Field 1) constraints
...
- **Prerequisites:** Day 09 base lab manual + Field 1 environment setup
...
- **Key Concepts:** Field-optimized Routing Protocols Intro, Black Start-specific validation, proof obligations, real-world constraints

## 1. Business Context

Haiti P38 pilot sites experience frequent power losses (avg. 6 hours/day) and unreliable internet.
This lab validates that Routing Protocols Intro works in **offline-only mode**:
...
**Why this matters:** Operators in remote areas cannot rely on central management. Routing Protocols Intro must work autonomously.
...
- Test: Can Routing Protocols Intro survive power loss? Is state recovered from cache?
...
- Test Routing Protocols Intro in "dark mode" (no external dependencies)
...
|--------|------|--------|---------|------|-------|
| R1 | Primary | 10.0.09.0/24 | 10.0.09.1 | /24 | Field 1 core router |
...
Router(config-if)# ip address 10.0.09.1 255.255.255.0
Router(config-if)# no shutdown
Router(config-if)# exit

! Disable routing protocol updates (no dynamic routing for offline)
! Static routes only
Router(config)# ip route 10.0.09.0 255.255.255.0 10.0.09.1
...
PC1# ping 10.0.09.10
Reply from 10.0.09.10: bytes=32 time=5ms
...
**Why offline-only topology for Routing Protocols Intro?**
...
The offline-only design proves Routing Protocols Intro works **without** these dependencies:
...
- Validates: Routing Protocols Intro is truly offline-capable for Haiti deployment
...
4. Routing Protocols Intro works autonomously without cloud sync
...
- Success: Routing Protocols Intro validated for real Haiti deployment
