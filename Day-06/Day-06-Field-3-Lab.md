# Day 06: Switch Configuration (Field 3: DePIN)

## 0. Metadata

- **Objective:** Prove Switch Configuration functionality in DePIN (Field 3) constraints
...
- **Prerequisites:** Day 06 base lab manual + Field 3 environment setup
...
- **Key Concepts:** Field-optimized Switch Configuration, DePIN-specific validation, proof obligations, real-world constraints

## 1. Business Context

DePIN (Decentralized Physical Infrastructure Networks) rely on **mesh topologies** without central authority.
This lab proves Switch Configuration works in **full-mesh, Byzantine-fault-tolerant** design:
- No central hub or single point of failure
- Nodes must reach consensus on Switch Configuration without a leader
- Byzantine nodes (malicious or failed) must not break the network
- Voting quorum (n/2 + 1) must validate all state changes

**Why this matters:** Decentralized networks cannot rely on centralized control. Switch Configuration must be fault-tolerant at the core.
...
- Test Switch Configuration with Byzantine nodes (R4 offline or lying)
...
- Implement voting/consensus for Switch Configuration decisions
...
|--------|------|--------|---------|------|-------|
| R1 | Primary | 10.0.06.0/24 | 10.0.06.1 | /24 | Field 3 core router |
...
Router(config-if)# ip address 10.0.06.11 255.255.255.0
Router(config-if)# no shutdown
Router(config-if)# exit

Router(config)# int g0/2
Router(config-if)# ip address 10.0.06.21 255.255.255.0
Router(config-if)# no shutdown
Router(config-if)# exit

! Implement voting/consensus (pseudo-code):
! if (consensus_check(Switch Configuration) >= n/2 + 1):
!     accept_update()
! else:
!     reject_update()
...
C 10.0.06.21/24 is directly connected, GigabitEthernet0/2
...
PC1# ping 10.0.06.10
Reply from 10.0.06.10: bytes=32 time=7ms
...
**Why mesh topology for Switch Configuration?**
...
**This lab validates:** Switch Configuration works in Byzantine-fault-tolerant mesh
