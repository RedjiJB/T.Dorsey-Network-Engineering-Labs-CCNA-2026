# Day 01: Network Models & OSI Reference (Field 3: DePIN)

## 0. Metadata

- **Objective:** Prove Network Models & OSI Reference functionality in DePIN (Field 3) constraints
...
- **Prerequisites:** Day 01 base lab manual + Field 3 environment setup
...
- **Key Concepts:** Field-optimized Network Models & OSI Reference, DePIN-specific validation, proof obligations, real-world constraints

## 1. Business Context

DePIN (Decentralized Physical Infrastructure Networks) rely on **mesh topologies** without central authority.
This lab proves Network Models & OSI Reference works in **full-mesh, Byzantine-fault-tolerant** design:
- No central hub or single point of failure
- Nodes must reach consensus on Network Models & OSI Reference without a leader
- Byzantine nodes (malicious or failed) must not break the network
- Voting quorum (n/2 + 1) must validate all state changes

**Why this matters:** Decentralized networks cannot rely on centralized control. Network Models & OSI Reference must be fault-tolerant at the core.
...
- Test Network Models & OSI Reference with Byzantine nodes (R4 offline or lying)
...
- Implement voting/consensus for Network Models & OSI Reference decisions
...
|--------|------|--------|---------|------|-------|
| R1 | Primary | 10.0.01.0/24 | 10.0.01.1 | /24 | Field 3 core router |
...
Router(config-if)# ip address 10.0.01.11 255.255.255.0
Router(config-if)# no shutdown
Router(config-if)# exit

Router(config)# int g0/2
Router(config-if)# ip address 10.0.01.21 255.255.255.0
Router(config-if)# no shutdown
Router(config-if)# exit

! Implement voting/consensus (pseudo-code):
! if (consensus_check(Network Models & OSI Reference) >= n/2 + 1):
!     accept_update()
! else:
!     reject_update()
...
C 10.0.01.21/24 is directly connected, GigabitEthernet0/2
...
PC1# ping 10.0.01.10
Reply from 10.0.01.10: bytes=32 time=7ms
...
**Why mesh topology for Network Models & OSI Reference?**
...
**This lab validates:** Network Models & OSI Reference works in Byzantine-fault-tolerant mesh
