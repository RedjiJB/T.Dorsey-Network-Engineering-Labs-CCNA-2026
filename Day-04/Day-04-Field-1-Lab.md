# Day 04: IPv4 Routing Fundamentals (Field 1: Black Start)

## 0. Metadata

- **Objective:** Prove IPv4 Routing Fundamentals functionality in Black Start (Field 1) constraints
- **Research Field:** Field 1: Black Start
- **Proof Obligations:**
  - Validate IPv4 Routing Fundamentals works without external dependencies (Field 1) / under stress (Field 2) / in mesh topology (Field 3) / at Haiti scale (Field 7)
...
- **Prerequisites:** Day 04 base lab manual + Field 1 environment setup
...
- **Key Concepts:** Field-optimized IPv4 Routing Fundamentals, Black Start-specific validation, proof obligations, real-world constraints

## 1. Business Context

Haiti P38 pilot sites experience frequent power losses (avg. 6 hours/day) and unreliable internet.
This lab validates that IPv4 Routing Fundamentals works in **offline-only mode**:
...
**Why this matters:** Operators in remote areas cannot rely on central management. IPv4 Routing Fundamentals must work autonomously.
...
- Test: Can IPv4 Routing Fundamentals survive power loss? Is state recovered from cache?
...
- Test IPv4 Routing Fundamentals in "dark mode" (no external dependencies)
...
|--------|------|--------|---------|------|-------|
| R1 | Primary | 10.0.04.0/24 | 10.0.04.1 | /24 | Field 1 core router |
| R2 | Secondary | 10.0.04.0/24 | 10.0.04.2 | /24 | Backup/mesh peer |
| SW1 | Access | 10.0.04.0/24 | 10.0.04.254 | /24 | Access switch VLAN |
| PC1 | Client | 10.0.04.0/24 | 10.0.04.10 | /24 | Test client 1 |
| PC2 | Client | 10.0.04.0/24 | 10.0.04.20 | /24 | Test client 2 |

**Field 1 specific notes:**
- Subnets chosen to test Day 04 concepts in isolation
...
```
! Standard initial configuration for Day 04: IPv4 Routing Fundamentals
Router> en
Router# conf t
...
Router(config-if)# ip address 10.0.04.1 255.255.255.0
Router(config-if)# no shutdown
Router(config-if)# exit

! Disable routing protocol updates (no dynamic routing for offline)
! Static routes only
Router(config)# ip route 10.0.04.0 255.255.255.0 10.0.04.1
...
Router# show ip route
C 10.0.04.0/24 is directly connected, GigabitEthernet0/0
...
PC1# ping 10.0.04.10
Reply from 10.0.04.10: bytes=32 time=5ms
...
GigabitEthernet0/0         10.0.04.1    YES manual up                    up
GigabitEthernet0/1         10.0.04.11   YES manual up                    up
Serial0/0                  10.0.04.2    YES manual up                    up
```
...
C     10.0.04.0/24 is directly connected, GigabitEthernet0/0
C     10.0.04.11/24 is directly connected, GigabitEthernet0/1
C     10.0.04.2/24 is directly connected, Serial0/0
```

### 7.3 Ping Output (Normal and Under Stress)
```
PC1# ping 10.0.04.10 -c 5
Request sent from ICMP seq=1, timeout=20ms
Reply from 10.0.04.10: bytes=32 time=5ms TTL=63
Reply from 10.0.04.10: bytes=32 time=6ms TTL=63
Reply from 10.0.04.10: bytes=32 time=7ms TTL=63
Reply from 10.0.04.10: bytes=32 time=5ms TTL=63
Reply from 10.0.04.10: bytes=32 time=6ms TTL=63
...
**Why offline-only topology for IPv4 Routing Fundamentals?**
...
The offline-only design proves IPv4 Routing Fundamentals works **without** these dependencies:
...
- Validates: IPv4 Routing Fundamentals is truly offline-capable for Haiti deployment
...
4. IPv4 Routing Fundamentals works autonomously without cloud sync
...
- Success: IPv4 Routing Fundamentals validated for real Haiti deployment
