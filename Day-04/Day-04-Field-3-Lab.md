# Day 04: IPv4 Routing Fundamentals (Field 3: DePIN)

## 0. Metadata

- **Objective:** Prove IPv4 Routing Fundamentals functionality in DePIN (Field 3) constraints
...
- **Prerequisites:** Day 04 base lab manual + Field 3 environment setup
...
- **Key Concepts:** Field-optimized IPv4 Routing Fundamentals, DePIN-specific validation, proof obligations, real-world constraints

## 1. Business Context

DePIN (Decentralized Physical Infrastructure Networks) rely on **mesh topologies** without central authority.
This lab proves IPv4 Routing Fundamentals works in **full-mesh, Byzantine-fault-tolerant** design:
- No central hub or single point of failure
- Nodes must reach consensus on IPv4 Routing Fundamentals without a leader
- Byzantine nodes (malicious or failed) must not break the network
- Voting quorum (n/2 + 1) must validate all state changes

**Why this matters:** Decentralized networks cannot rely on centralized control. IPv4 Routing Fundamentals must be fault-tolerant at the core.
...
- Test IPv4 Routing Fundamentals with Byzantine nodes (R4 offline or lying)
...
- Implement voting/consensus for IPv4 Routing Fundamentals decisions
...
|--------|------|--------|---------|------|-------|
| R1 | Primary | 10.0.04.0/24 | 10.0.04.1 | /24 | Field 3 core router |
...
Router(config-if)# ip address 10.0.04.11 255.255.255.0
Router(config-if)# no shutdown
Router(config-if)# exit

Router(config)# int g0/2
Router(config-if)# ip address 10.0.04.21 255.255.255.0
Router(config-if)# no shutdown
Router(config-if)# exit

! Implement voting/consensus (pseudo-code):
! if (consensus_check(IPv4 Routing Fundamentals) >= n/2 + 1):
!     accept_update()
! else:
!     reject_update()
...
C 10.0.04.21/24 is directly connected, GigabitEthernet0/2
...
PC1# ping 10.0.04.10
Reply from 10.0.04.10: bytes=32 time=7ms
...
**Why mesh topology for IPv4 Routing Fundamentals?**
...
**This lab validates:** IPv4 Routing Fundamentals works in Byzantine-fault-tolerant mesh
