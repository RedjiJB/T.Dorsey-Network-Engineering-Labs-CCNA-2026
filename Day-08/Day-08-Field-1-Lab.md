# Day 08: Static Routing (Field 1: Black Start)

## 0. Metadata

- **Objective:** Prove Static Routing functionality in Black Start (Field 1) constraints
...
- **Prerequisites:** Day 08 base lab manual + Field 1 environment setup
...
- **Key Concepts:** Field-optimized Static Routing, Black Start-specific validation, proof obligations, real-world constraints

## 1. Business Context

Haiti P38 pilot sites experience frequent power losses (avg. 6 hours/day) and unreliable internet.
This lab validates that Static Routing works in **offline-only mode**:
...
**Why this matters:** Operators in remote areas cannot rely on central management. Static Routing must work autonomously.
...
- Test: Can Static Routing survive power loss? Is state recovered from cache?
...
- Test Static Routing in "dark mode" (no external dependencies)
...
|--------|------|--------|---------|------|-------|
| R1 | Primary | 10.0.08.0/24 | 10.0.08.1 | /24 | Field 1 core router |
...
! Static routes only
Router(config)# ip route 10.0.08.0 255.255.255.0 10.0.08.1
...
PC1# ping 10.0.08.10
Reply from 10.0.08.10: bytes=32 time=5ms
...
**Why offline-only topology for Static Routing?**
...
The offline-only design proves Static Routing works **without** these dependencies:
...
- Validates: Static Routing is truly offline-capable for Haiti deployment
...
4. Static Routing works autonomously without cloud sync
...
- Success: Static Routing validated for real Haiti deployment
