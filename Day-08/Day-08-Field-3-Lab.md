# Day 08: Static Routing (Field 3: DePIN)

## 0. Metadata

- **Objective:** Prove Static Routing functionality in DePIN (Field 3) constraints
- **Research Field:** Field 3: DePIN
- **Proof Obligations:**
  - Validate Static Routing works without external dependencies (Field 1) / under stress (Field 2) / in mesh topology (Field 3) / at Haiti scale (Field 7)
...
- **Prerequisites:** Day 08 base lab manual + Field 3 environment setup
...
- **Key Concepts:** Field-optimized Static Routing, DePIN-specific validation, proof obligations, real-world constraints

## 1. Business Context

DePIN (Decentralized Physical Infrastructure Networks) rely on **mesh topologies** without central authority.
This lab proves Static Routing works in **full-mesh, Byzantine-fault-tolerant** design:
- No central hub or single point of failure
- Nodes must reach consensus on Static Routing without a leader
- Byzantine nodes (malicious or failed) must not break the network
- Voting quorum (n/2 + 1) must validate all state changes

**Why this matters:** Decentralized networks cannot rely on centralized control. Static Routing must be fault-tolerant at the core.
...
- Test Static Routing with Byzantine nodes (R4 offline or lying)
...
- Implement voting/consensus for Static Routing decisions
...
|--------|------|--------|---------|------|-------|
| R1 | Primary | 10.0.08.0/24 | 10.0.08.1 | /24 | Field 3 core router |
| R2 | Secondary | 10.0.08.0/24 | 10.0.08.2 | /24 | Backup/mesh peer |
| SW1 | Access | 10.0.08.0/24 | 10.0.08.254 | /24 | Access switch VLAN |
| PC1 | Client | 10.0.08.0/24 | 10.0.08.10 | /24 | Test client 1 |
| PC2 | Client | 10.0.08.0/24 | 10.0.08.20 | /24 | Test client 2 |

**Field 3 specific notes:**
- Subnets chosen to test Day 08 concepts in isolation
...
```
! Standard initial configuration for Day 08: Static Routing
Router> en
Router# conf t
...
Router(config-if)# ip address 10.0.08.1 255.255.255.0
Router(config-if)# no shutdown
Router(config-if)# exit

! Add connections to other mesh peers
Router(config)# int g0/1
Router(config-if)# ip address 10.0.08.11 255.255.255.0
Router(config-if)# no shutdown
Router(config-if)# exit

Router(config)# int g0/2
Router(config-if)# ip address 10.0.08.21 255.255.255.0
Router(config-if)# no shutdown
Router(config-if)# exit

! Implement voting/consensus (pseudo-code):
! if (consensus_check(Static Routing) >= n/2 + 1):
!     accept_update()
! else:
!     reject_update()
...
Router# show ip route
C 10.0.08.0/24 is directly connected, GigabitEthernet0/0
C 10.0.08.11/24 is directly connected, GigabitEthernet0/1
C 10.0.08.21/24 is directly connected, GigabitEthernet0/2
...
PC1# ping 10.0.08.10
Reply from 10.0.08.10: bytes=32 time=7ms
...
GigabitEthernet0/0         10.0.08.1    YES manual up                    up
GigabitEthernet0/1         10.0.08.11   YES manual up                    up
Serial0/0                  10.0.08.2    YES manual up                    up
```
...
C     10.0.08.0/24 is directly connected, GigabitEthernet0/0
C     10.0.08.11/24 is directly connected, GigabitEthernet0/1
C     10.0.08.2/24 is directly connected, Serial0/0
```

### 7.3 Ping Output (Normal and Under Stress)
```
PC1# ping 10.0.08.10 -c 5
Request sent from ICMP seq=1, timeout=20ms
Reply from 10.0.08.10: bytes=32 time=5ms TTL=63
Reply from 10.0.08.10: bytes=32 time=6ms TTL=63
Reply from 10.0.08.10: bytes=32 time=7ms TTL=63
Reply from 10.0.08.10: bytes=32 time=5ms TTL=63
Reply from 10.0.08.10: bytes=32 time=6ms TTL=63
...
**Why mesh topology for Static Routing?**
...
**This lab validates:** Static Routing works in Byzantine-fault-tolerant mesh
